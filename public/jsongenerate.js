...
const markdownfiles = path.join(__dirname, "../src/content");
let postlist = [];
...
const getPosts = () => {
  fs.readdir(markdownfiles, (err, files) => {
    if (err) {
      return console.log(
        `An error has occured and this is the path: ${markdownfiles}`
      );
    }
    ...
    files.forEach((file, i) => {
      ...
      fs.readFile(`${markdownfiles}/${file}`, "utf-8", (err, contents) => {
        ...
        if (i === files.length - 1) {
          ...
          fs.writeFileSync('src/routes/blog/posts.json', data);
        }
      });
    });
  });
